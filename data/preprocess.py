...
def prep_pointcloud(input_dict,
    ...
    if training:
        ...
        #cv2.imwrite('/root/second.pytorch/images/kitti_bev/pre_noise'+str(input_dict['metadata']['image_idx'])+'.jpg', bev_map)
        ...
        #boxes_lidar = gt_dict["gt_boxes"]
        #bev_map = simplevis.kitti_vis(points, boxes_lidar)
        #cv2.imwrite('/root/second.pytorch/images/kitti_bev/post_noise'+str(input_dict['metadata']['image_idx'])+'.jpg', bev_map)

        # boxes_lidar = gt_dict["gt_boxes"]
        # bev_map = simplevis.nuscene_vis(points, boxes_lidar)
        # cv2.imshow('post-noise', bev_map)
        # cv2.waitKey(0)
    # prepare gt_class ,gt_bbox for evaluation, check the code to make sure gt_data is unmodified 
    else: 
        ...
        #pass the needed class and bbox for fcos , and make data later , need to find better port to do this
    ...
    voxels, coordinates, num_points = voxel_generator.generate(
        points, max_voxels)
    #fcos add gt_class, gt_bbox for making target latter
    example = {
        'points' : points,
        'voxels': voxels,
        ...
        "gt_class":gt_dict["gt_classes"],
        "gt_bbox":gt_dict["gt_boxes"],
    ...
    return example
    # return here , below is for original code, build target before trainging , it's slow
    ...
    """
    #use FCOS , no need to create target here
