...
class VoxelFeatureExtractorV4(nn.Module):
    ...
    def forward(self, features, num_voxels, coors):
        ...
        points_var = features.var(dim=1, keepdim=True).permute(0, 2, 1).squeeze(dim=2).size().type(torch.cuda.FloatTensor)

        valid_points_in_voxel = torch.unsqueeze((features.size()[1] -  (features.sum(dim=2)==0).sum(dim=1)),1)

        voxel_feature = torch.cat([points_mean,points_var.type(torch.cuda.FloatTensor), valid_points_in_voxel.type(torch.cuda.FloatTensor)], dim=-1)



        return voxel_feature.contiguous()
