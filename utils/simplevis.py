...
def kitti_vis(points, pred_boxes, gt_boxes, labels=None):
    vis_voxel_size = [0.1, 0.1, 0.1]
    vis_point_range = [0, -40, -3, 70.4, 40, 1]
    bev_map = point_to_vis_bev(points, vis_voxel_size, vis_point_range)
    bev_map = draw_box_in_bev(bev_map, vis_point_range, pred_boxes, [0, 255, 0], 2, labels)
    bev_map = draw_box_in_bev(bev_map, vis_point_range, gt_boxes, [0,0 , 255], 1, labels)
    return bev_map
