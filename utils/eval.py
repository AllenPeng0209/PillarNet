...
def compute_statistics_jit(overlaps,
    ...
    delta_idx = 0
    percent_error_x_list = []
    percent_error_y_list = []
    percent_error_w_list = []
    percent_error_l_list = []
    percent_error_zr_list = []
    ...
    for i in range(gt_size):
        ...
        if (valid_detection == NO_DETECTION) and ignored_gt[i] == 0:
            ...
            if compute_percent_error:
                
                #mae
                percent_error_x = abs(gt_datas[i][0] - dt_datas[det_idx][0])
                percent_error_y = abs(gt_datas[i][1] - dt_datas[det_idx][1])
                percent_error_w = abs((gt_datas[i][2] - dt_datas[det_idx][2])) 
                percent_error_l = abs((gt_datas[i][3] - dt_datas[det_idx][3]))
                percent_error_zr = abs((gt_datas[i][4] - dt_datas[det_idx][4])) 

                #mape 
                #percent_error_x = abs((gt_datas[i][0] - dt_datas[det_idx][0]) / (gt_datas[i][0]+1e-10))
                #percent_error_y = abs((gt_datas[i][1] - dt_datas[det_idx][1]) / (gt_datas[i][1]+1e-10))
                #percent_error_w = abs((gt_datas[i][2] - dt_datas[det_idx][2]) / (gt_datas[i][2]+1e-10))
                #percent_error_l = abs((gt_datas[i][3] - dt_datas[det_idx][3]) / (gt_datas[i][3]+1e-10))
                #percent_error_zr = abs((gt_datas[i][4] - dt_datas[det_idx][4]) / (gt_datas[i][4]+1e-10))
                
                #print(percent_error)
                percent_error_x_list.append(percent_error_x)
                percent_error_y_list.append(percent_error_y)
                percent_error_w_list.append(percent_error_w)
                percent_error_l_list.append(percent_error_l)
                percent_error_zr_list.append(percent_error_zr)
            if compute_aos:
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    if compute_percent_error:
        if len(percent_error_x_list)!=0 : 
                
            percent_error_x_mean = np.array(percent_error_x_list).sum()
            percent_error_y_mean = np.array(percent_error_y_list).sum()
            percent_error_w_mean = np.array(percent_error_w_list).sum()
            percent_error_l_mean = np.array(percent_error_l_list).sum()
            percent_error_zr_mean = np.array(percent_error_zr_list).sum()           
        else: 
            percent_error_x_mean = -1
            percent_error_y_mean = -1
            percent_error_w_mean = -1
            percent_error_l_mean = -1
            percent_error_zr_mean = -1
    #return tp, fp, fn, similarity, thresholds[:thresh_idx]
    return tp, fp, fn, similarity, thresholds[:thresh_idx], percent_error_x_mean, percent_error_y_mean, percent_error_w_mean, percent_error_l_mean, percent_error_zr_mean
...
def fused_compute_statistics(overlaps,
    ...
    for i in range(gt_nums.shape[0]):
        for t, thresh in enumerate(thresholds):
            ...
            tp, fp, fn, similarity, _ , percent_error_x_mean,percent_error_y_mean,percent_error_w_mean,percent_error_l_mean,percent_error_zr_mean  = compute_statistics_jit(
            ...
            if compute_percent_error: 
                if percent_error_x_mean != -1:
                    pr[t, 4 ]+= percent_error_x_mean
                    pr[t, 5 ]+= percent_error_y_mean
                    pr[t, 6 ]+= percent_error_w_mean
                    pr[t, 7 ]+= percent_error_l_mean
                    pr[t, 8 ]+= percent_error_zr_mean
...
def eval_class_v3(gt_annos,
    ...
    mape_center_x = np.zeros([num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
    mape_center_y = np.zeros([num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
    mape_center_w = np.zeros([num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
    mape_center_l = np.zeros([num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
    mape_center_zr = np.zeros([num_class, num_difficulty, num_minoverlap, N_SAMPLE_PTS])
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    ...
                    tp, fp, fn, similarity, thresholds, percent_error_x_mean,percent_error_y_mean,percent_error_w_mean,percent_error_l_mean,percent_error_zr_mean = rets
                ...
                for i in range(len(thresholds)):
                    ...
                    if compute_percent_error:
                        mape_center_x[m,l,k,i] =  pr[i, 4]/  pr[i, 0]     
                        mape_center_y[m,l,k,i] =  pr[i, 5] /  pr[i, 0]    
                        mape_center_w[m,l,k,i] =  pr[i, 6] /  pr[i, 0]    
                        mape_center_l[m,l,k,i] =  pr[i, 7] /  pr[i, 0]    
                        mape_center_zr[m,l,k,i] =  pr[i, 8] /  pr[i, 0]    
                for i in range(len(thresholds)):
                    ...
                    if compute_percent_error:
                        mape_center_x[m,l,k,i] =   np.max(mape_center_x[m,l,k,i], axis=-1)
                        mape_center_y[m,l,k,i] =   np.max(mape_center_y[m,l,k,i], axis=-1)
                        mape_center_w[m,l,k,i] =   np.max(mape_center_w[m,l,k,i], axis=-1)
                        mape_center_l[m,l,k,i] =   np.max(mape_center_l[m,l,k,i], axis=-1)
                        mape_center_zr[m,l,k,i] =   np.max(mape_center_zr[m,l,k,i], axis=-1)
    ...
    ret_dict = {
        ...
        "mape_center_x":mape_center_x,
        "mape_center_y":mape_center_y,
        "mape_center_w":mape_center_w,
        "mape_center_l":mape_center_l,
        "mape_center_zr":mape_center_zr
...
def get_official_eval_result(gt_annos,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if compute_percent_error:
                mae_x = metrics["bbox"]["mape_center_x"][j, :, i].mean(axis=1)
                detail[class_name][f"mae_x"] = mae_x.tolist()
                mae_x = ", ".join(f"{v:.2f}" for v in mae_x)
                result += print_str(f"x  mae:{mae_x}")
                
                mae_y = metrics["bbox"]["mape_center_y"][j, :, i].mean(axis=1)
                detail[class_name][f"mae_y"] = mae_y.tolist()
                mae_y = ", ".join(f"{v:.2f}" for v in mae_y)
                result += print_str(f"y  mae:{mae_y}")
                
                mae_w = metrics["bbox"]["mape_center_w"][j, :, i].mean(axis=1)
                detail[class_name][f"mae_w"] = mae_w.tolist()
                mae_w = ", ".join(f"{v:.2f}" for v in mae_w)
                result += print_str(f"w  mae:{mae_w}")
                
                mae_l = metrics["bbox"]["mape_center_l"][j, :, i].mean(axis=1)
                detail[class_name][f"mae_l"] = mae_l.tolist()
                mae_l = ", ".join(f"{v:.2f}" for v in mae_l)
                result += print_str(f"l  mae:{mae_l}")

                mae_zr = metrics["bbox"]["mape_center_zr"][j, :, i].mean(axis=1)
                detail[class_name][f"mae_zr"] = mae_zr.tolist()
                mae_zr = ", ".join(f"{v:.2f}" for v in mae_zr)
                result += print_str(f"zr  mae:{mae_zr}")
    return {
        "result": result,
        "detail": detail,
    }
